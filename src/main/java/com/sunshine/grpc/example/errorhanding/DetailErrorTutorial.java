 ...
 * level error.
 */
public class DetailErrorTutorial {

    private static final Logger logger = LoggerFactory.getLogger(DetailErrorTutorial.class);
     ...
     * @param t
     */
    private void verifyErrorReply(Throwable t) {
    ...
    private void futureCallCallback() {
        GreeterGrpc.GreeterFutureStub stub = GreeterGrpc.newFutureStub(channel);
        ListenableFuture<HelloReply> response = stub.sayHello(HelloRequest.newBuilder().build());
    ...
    private void advancedAsyncCall() {
        ...
        call.start(new ClientCall.Listener<HelloReply>() {
            ...
            public void onClose(Status status, Metadata trailers) {
                Verify.verify(status.getCode().equals(Status.Code.INTERNAL));
                Verify.verify(trailers.containsKey(DEBUG_INFO_TRAILER_KEY));
